...
FixedSizeStack *FixedSizeStack_new(size_t buffer_size, error_t *error){
    ...
    new_stack -> buffer = (unsigned*)malloc(sizeof(unsigned) * buffer_size);
...
void FixedSizeStack_push(FixedSizeStack *stack, unsigned data, error_t *error){
    assert(NULL != error);
    if(NULL != error){
        *error = SUCCESS;
    }
...
unsigned FixedSizeStack_pop(FixedSizeStack *stack, error_t *error){
    assert(NULL != error);
    if(NULL != error){
        *error = SUCCESS;
    }
...
unsigned FixedSizeStack_peek(FixedSizeStack *stack, error_t *error){
    assert(NULL != error);
    if(NULL != error){
        *error = SUCCESS;
    }
